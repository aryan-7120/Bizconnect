...
/**
 * Business owner's personal profile page.
 * Separated from the Business Profile (which is for business details like address/category).
 * This page manages the owner user account: name, phone, avatar, password.
 */
        ...
        <div className="max-w-3xl mx-auto px-4 sm:px-6">
          <div className="flex items-center gap-4">
            <div className="relative">
              ...
              <label htmlFor="owner-avatar" className="absolute -bottom-2 -right-2 w-8 h-8 bg-white text-indigo-600 rounded-full flex items-center justify-center cursor-pointer shadow-lg hover:scale-110 transition-transform">
                <Camera className="w-4 h-4" />
                <input id="owner-avatar" type="file" accept="image/*" className="hidden" onChange={handleAvatarChange} />
              </label>
            </div>
            <div>
              <h1 className="text-2xl font-bold">{user?.name}</h1>
              <p className="text-indigo-200 text-sm">Business Owner Account</p>
            </div>
          </div>
        </div>
      </div>
            ...
            <form onSubmit={handleSaveProfile} className="space-y-5">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="email" value={user?.email || ''} className="input-field pl-10 opacity-60 cursor-not-allowed" disabled />
                </div>
              ...
              {avatarFile && (
                <div className="flex items-center gap-2 text-sm text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-900/20 px-4 py-3 rounded-xl">
                  <Camera className="w-4 h-4" /> New avatar: <strong>{avatarFile.name}</strong>
                </div>
              )}
